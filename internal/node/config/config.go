...
func LoadConfig(path string) (*Config, error) {
	...
	configloader.OverrideBool(&cfg.Cache.Enabled, "CACHE_ENABLED")
	configloader.OverrideInt(&cfg.Cache.HTTPPort, "CACHE_HTTP_PORT")
	configloader.OverrideInt(&cfg.Cache.CapacityMB, "CACHE_CAPACITY_MB")
	configloader.OverrideInt(&cfg.Cache.DefaultTTL, "CACHE_DEFAULT_TTL")
	configloader.OverrideFloat(&cfg.Cache.HotspotThreshold, "CACHE_HOTSPOT_THRESHOLD")
	configloader.OverrideFloat(&cfg.Cache.HotspotDecayRate, "CACHE_HOTSPOT_DECAY")

	// Apply defaults
	if cfg.Cache.HTTPPort == 0 {
		cfg.Cache.HTTPPort = 8080
	}
	if cfg.Cache.CapacityMB == 0 {
		cfg.Cache.CapacityMB = 1024 // 1GB default
	}
	if cfg.Cache.DefaultTTL == 0 {
		cfg.Cache.DefaultTTL = 3600 // 1 hour
	}
	if cfg.Cache.HotspotThreshold == 0 {
		cfg.Cache.HotspotThreshold = 1000.0
	}
	if cfg.Cache.HotspotDecayRate == 0 {
		cfg.Cache.HotspotDecayRate = 0.65
	}
	// Apply defaults
	if cfg.Node.Bind == "" {
...
func (cfg *Config) ValidateConfig() error {
	...
	if cfg.Cache.Enabled {
		if cfg.Cache.HTTPPort < 1024 || cfg.Cache.HTTPPort > 65535 {
			errs = append(errs, fmt.Sprintf("cache.httpPort must be in [1024,65535], got %d", cfg.Cache.HTTPPort))
		}
		if cfg.Cache.CapacityMB <= 0 {
			errs = append(errs, fmt.Sprintf("cache.capacityMB must be > 0, got %d", cfg.Cache.CapacityMB))
		}
		if cfg.Cache.DefaultTTL <= 0 {
			errs = append(errs, fmt.Sprintf("cache.defaultTTL must be > 0, got %d", cfg.Cache.DefaultTTL))
		}
		if cfg.Cache.HotspotThreshold <= 0 {
			errs = append(errs, fmt.Sprintf("cache.hotspotThreshold must be > 0, got %f", cfg.Cache.HotspotThreshold))
		}
		if cfg.Cache.HotspotDecayRate <= 0 || cfg.Cache.HotspotDecayRate >= 1 {
			errs = append(errs, fmt.Sprintf("cache.hotspotDecayRate must be in (0,1), got %f", cfg.Cache.HotspotDecayRate))
		}
	}
...
func (cfg *Config) LogConfig(lgr logger.Logger) {
	lgr.Info("Loaded configuration",
		...
		// Cache config
		logger.F("cache.enabled", cfg.Cache.Enabled),
		logger.F("cache.httpPort", cfg.Cache.HTTPPort),
		logger.F("cache.capacityMB", cfg.Cache.CapacityMB),
		logger.F("cache.defaultTTL", cfg.Cache.DefaultTTL),
		logger.F("cache.hotspotThreshold", cfg.Cache.HotspotThreshold),
		logger.F("cache.hotspotDecayRate", cfg.Cache.HotspotDecayRate),
	)
}
