...
//
// This implements Section 4.2 of the Koorde paper for fault-tolerant de Bruijn
// pointer maintenance. Instead of finding predecessor(k×m), it finds
// predecessor(k×m - x) where x = O(log n/n), ensuring that O(log n) nodes
// occupy the interval and provide fault tolerance.
//
// The procedure is:
//  1. Estimate the network size n
//  2. Compute the fault-tolerant target = (k×m - x) mod 2^b
//  3. Find the anchor as the predecessor of the target
//  4. Set digit 0 of the de Bruijn window to the anchor
//  5. Fill remaining digits with entries from the anchor's successor list
...
func (n *Node) fixDeBruijn() {
	...
	n.lgr.Debug("fixDeBruijn: computed fault-tolerant target",
		...
		logger.F("estimated_network_size", estimatedN))
