...
// EstimateNetworkSize estimates the number of nodes in the network based on
// the distance to the first successor.
//
// The estimation assumes nodes are uniformly distributed in the identifier space.
// If the distance to the first successor is d, then the estimated number of nodes
// is approximately 2^Bits / d.
//
// Returns:
//   - Estimated network size (minimum 1)
...
func (n *Node) FindSuccessorInit(ctx context.Context, target domain.ID) (*domain.Node, error) {
	...
	// In FindSuccessorInit, line 121:
	currentI, kshift, err := n.rt.Space().BestImaginary(self.ID, succ.ID, target)
	if err != nil {
